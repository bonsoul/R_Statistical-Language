...
df4 <- read_csv("D:/Downloads/Final Household data Edit.csv")
df5 <- read_csv("D:/Downloads/HCW Excel Final Edit.csv")
df6 <- read_csv("D:/Downloads/Prison Excel Final Edit.csv")
...
# Double check
table(df18$HBV_binary, df18$HBV_Status)
...
# Logistic regression
model_sex <- glm(HBV_binary ~ N2Sex1, data = df18, family = binomial)
...
# Double check
table(df19$HBV_binary, df19$HBV_Status)
```
...
# Display the results in a nice formatted table
results_firth %>%
  kable(
    caption = "Table: Adjusted Odds Ratios (aOR) from Firth Logistic Regression",
    align = c("l", "c"),  # left-align variable, center aOR column
    col.names = c("Variable", "Adjusted OR (95% CI)")
  ) %>%
  kable_styling(
    bootstrap_options = c("striped", "hover", "condensed", "responsive"),
    full_width = FALSE,
...
combined_plot2 <- (map_tested + map_positive + map_negative) /
  (map_vaccinated + map_hcw_vax + map_rate) +
  plot_annotation(
    title = "Hepatitis B Indicators by Subcounty — Baringo County",
    caption = "Source: Field Data & Kenya Open Data | Map: Bonsoul",
    theme = theme(
...
```{r, include=TRUE}
make_map5 <- function(data, fill_var, title) {
  ggplot(data) +
    geom_sf(aes(fill = .data[[fill_var]]), color = "white", linewidth = 0.4) +
    coord_sf(datum = NA) +
    scale_fill_viridis_c(option = "plasma", direction = -1, na.value = "gray85") +
    geom_sf_text(aes(label = subcounty_clean),
                 size = 4, color = "black", fontface = "bold", check_overlap = TRUE) +
    labs(title = title) +
    theme_minimal(base_size = 14) +
    theme(
      plot.background = element_rect(fill = "gray92", color = NA),
      panel.background = element_rect(fill = "gray92", color = NA),
      legend.background = element_rect(fill = "gray92", color = NA),
      legend.key = element_rect(fill = "gray92", color = NA),
...
```{r, include=FALSE}
# Print each thematic map separately
map_tested     <- make_map5(baringo_data, "No_Tested", "No. Tested")
map_positive   <- make_map5(baringo_data, "No_Positive", "No. Positive")
map_negative   <- make_map5(baringo_data, "No_Negative", "No. Negative")
map_vaccinated <- make_map5(baringo_data, "Total_Vaccinated", "Total Vaccinated")
map_hcw_vax    <- make_map5(baringo_data, "HCW_Vaccinated", "HCWs Vaccinated")
map_rate       <- make_map5(baringo_data, "Positivity_Rate", "Positivity Rate (%)")
