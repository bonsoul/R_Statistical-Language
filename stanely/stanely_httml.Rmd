...
```{r Count the number of facilitating, include=FALSE}
...
summarize_demo <- function(data, var, var_name, labels) {
  data %>%
    ...
    mutate(
      Variable = var_name,
      Category = factor(as.character({{ var }}), levels = names(labels), labels = labels),
      Percentage = round(100 * n / sum(n), 1)
    ) %>%
    select(Variable, Category, Count = n, Percentage)
}

# Apply function to different demographic variables
...
qualification_tbl <- summarize_demo(df4, qualification, "Qualification", c("1" = "Diploma", "2" = "Bachelor", "3" = "Other"))
...
demographics_summary %>%
  ...
  kable(caption = "Table: Summary of Demographic Characteristics of Respondents") %>%
...
table1_labeled <- table1_cleaned %>%
  mutate(
    Category = case_when(
      ...
      Group == "Qualification" & Category == 3 ~ "Others",
...
kable(table1_filtered, format = "html", 
      caption = "Summary of Healthcare Professionals’ Knowledge and Attitudes on Herbal Medicine") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = FALSE) %>%
...
```{r}
# Assuming these are the utilization variables
utilization_vars1 <- df4 %>% select(util_always,util_often,util_sometimes)

# Summarize each utilization question
util_summary <- bind_rows(
  lapply(names(utilization_vars1), function(var) {
    df4 %>%
      count(!!sym(var)) %>%
      mutate(
        Question = var,
        Response = as.character(!!sym(var)),
        Percentage = round(100 * n / sum(n), 1)
      ) %>%
      select(Question, Response, Count = n, Percentage)
  })
)
...
# Display as Harvard-style table
kable(table2_labeled, format = "html", 
      caption = "PLHIV and TB Burden as Percent of County Population") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = FALSE) %>%
  column_spec(1, bold = TRUE)
...
```{r chi_square, include=TRUE}

fisher_result <- fisher.test(table(df6$knowledge_level, df6$attitude_total_score),
                             simulate.p.value = TRUE, B = 10000)

# Extract values from the htest object
fisher_df <- data.frame(
  Test = "Fisher's Exact Test",
  P_Value = format.pval(fisher_result$p.value, digits = 4),
  Alternative = fisher_result$alternative,
  Method = fisher_result$method,
  stringsAsFactors = FALSE
)

kable(fisher_df, format = "html", caption = "Fisher’s Exact Test Results") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = FALSE) %>%
  column_spec(1, bold = TRUE)
...
```{r}
write.csv(df6, "df_output.csv", row.names = FALSE)
...
# Extract values from the cor.test object
cor_df <- data.frame(
  Test = "Spearman Correlation",
  Correlation_Coefficient = round(cor_result$estimate, 3),
  P_Value = format.pval(cor_result$p.value, digits = 4),
...
kable(cor_df, format = "html", caption = "Spearman Correlation Test Results") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), 
                full_width = FALSE) %>%
  column_spec(1, bold = TRUE)
