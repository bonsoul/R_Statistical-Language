...
# Remove "Sub County" from homabay_shp$subcounty
homabay_shp$subcounty <- gsub(" Sub County", "", homabay_shp$subcounty)

# Remove "Sub County" from hiv_counts$subcounty
hiv_counts$subcounty <- gsub(" Sub County", "", hiv_counts$subcounty)

# Check unique values after modification
...
ggplot(data = homabay_shp) +
  ...
  scale_fill_viridis_c(option = "plasma", name = "Total HIV Cases", na.value = "grey80") +
  labs(
...
colnames(df_sf)  # Should now include 'total_cases'
...
ggplot(data = df_sf) +
  ...
  scale_fill_viridis_c(option = "plasma", name = "Total HIV Cases", na.value = "grey80") +
  labs(
    title = "Geographical Distribution of HIV Cases in Sub-Counties",
    subtitle = "Visualization of HIV patient distribution",
    x = "Longitude",
    y = "Latitude"
  ) +
  theme_minimal() +
...
homabay_shp <- homabay_shp %>%
  left_join(patient_count, by = "subcounty")
  
  colnames(homabay_shp)
  
  homabay_shp %>%
  select(subcounty, Number_Patients) %>%
  st_drop_geometry() %>%
  head(10) %>%
  kable()
...
name_mapping <- data.frame(
  patient_counts_name = c("Homa Bay", "Kabondo Kasipul", "Karachauony", 
                      "Kasipul", "Mbita", "Ndhiwa", "Ragwe", "Suba"),
  homabay_shp_name = c("Homa Bay Town Sub County", "Kabondo Kasipul Sub County",
                       "Karachuonyo Sub County", "Kasipul Sub County", 
                       "Mbita Sub County", "Ndhiwa Sub County", 
                       "Rangwe Sub County", "Suba Sub County")
)

patient_cases <- patient_count %>%
  left_join(name_mapping, by = c("subcounty" = "patient_counts_name")) %>%
  mutate(subcounty = homabay_shp_name) %>%
  select(-homabay_shp_name)
...
ggplot(data = homabay_shp) +
  geom_sf(aes(fill = Number_Patients), color = "black", size = 0.3) +
  scale_fill_viridis_c(option = "plasma", name = "Total HIV Cases", na.value = "grey80") +
  labs(
    title = "Geographical Distribution of HIV Cases in Homa Bay",
    subtitle = "Sub-county level analysis",
